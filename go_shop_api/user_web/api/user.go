...
func PassWordLogin(ctx *gin.Context) {
	...
	// 校验验证码
	// 这个逻辑登录不了if , 这个false是指是否清楚:clear
	// store.Verify(passWordLoginForm.CaptchaId, passWordLoginForm.Captcha, false) {
	// 	ctx.JSON(http.StatusBadRequest, gin.H{
	// 		"captcha": "验证码错误",
	// 	})
	// 	return
	...
	if rsp, err := global.UserSrvClient.GetUserByMobile(ctx, &proto.MobileRequest{
		...
		if passRsp, pasErr := global.UserSrvClient.CheckPassword(ctx, &proto.PasswordCheckInfo{
